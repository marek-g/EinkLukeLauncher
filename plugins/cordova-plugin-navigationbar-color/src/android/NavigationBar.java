        ...
        if (Build.VERSION.SDK_INT >= 21) {
            if (colorPref != null && !colorPref.isEmpty()) {
                ...
                try 
                {
                    ...
                    //Navbar...
					if(colorPref.equals("#set_nav"))
                    {
						window.setFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS, WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);				
					}
					
					//Different Statusbar types => different flags 
                    if(colorPref.equals("#set_trans1"))
                    {
						window.addFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);
					}
					
					
					if(colorPref.equals("#set_trans2"))
                    {				
					...
					if(colorPref.equals("#unset_trans2"))
                    {				
					...
					if(colorPref.equals("#unset_trans1"))
                    {
					...
					if(colorPref.equals("#set_padding"))
                    {				
					...
					if(colorPref.equals("#unset_padding"))
                    {				
						window.getDecorView().setPadding(0,0,0,0);
					}
					
                    // Using reflection makes sure any 5.0+ device will work without having to compile with SDK level 21
                    //window.getClass().getDeclaredMethod("setNavigationBarColor", int.class).invoke(window, Color.parseColor("#00ff0000"));
